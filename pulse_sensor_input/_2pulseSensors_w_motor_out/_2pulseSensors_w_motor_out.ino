...
boolean PHYSICAL_OUT = true;
boolean DIGITAL_OUT = false;
...
int pulsePinR = 4;  // pulse sensor purple wire connected to analog pin - RIGHT player
int pulsePinL = 5; //  pulse sensor purple wire connected to analog pin - LEFT player
...
if ((sampleCounter %20) == 0){// only look for the beat every 20mS. This clears out alot of high frequency noise.
  if (FSignalR < 0 && PulseR == false){  // signal surges down in value every time there is a pulse
     ...
     else if (PHYSICAL_OUT) {
       motor.step(1, FORWARD, SINGLE);	// send to motor
       //motor.release();
     }
  ...
  if (FSignalL < 0 && PulseL == false){  // signal surges down in value every time there is a pulse
     ...
     if (DIGITAL_OUT) {	
       ...
       //motor.step(100, FORWARD, DOUBLE);	// send to motor
     }
